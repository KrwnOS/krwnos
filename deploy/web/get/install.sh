...
#
# Этот файл — копия scripts/install.sh, которая хостится на
# get.krwnos.com (Apache + .htaccess в deploy/web/get/).
# При изменении канонического scripts/install.sh — обязательно
# перезалейте этот файл на shared-хостинг.
# -----------------------------------------------------------------------------
set -euo pipefail
