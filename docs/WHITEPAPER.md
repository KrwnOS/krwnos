...
**KrwnOS существует затем, чтобы у цифровых сообществ была собственная
юрисдикция в программном смысле слова** — не арендованный угол чужой
площадки, а **узел**, где закон, деньги и иерархия выражены **явно**,
**проверяемо** и **переносимо**. От клана до компании, от гильдии до
DAO: если людям нужны **общие правила и общий контур**, они заслуживают
инструмент, который **усиливает их самоопределение**, а не подменяет
его алгоритмом посредника или исчезновением аккаунта.

Мы проектируем это программное обеспечение как **фидуциарий**. Оно
**служит** Суверену и конституции его State и **не претендует** быть
моральным судьёй между гражданами, высшим законодателем внутри чужого
контура или скрытым арбитром через «особые возможности» платформы.
**Данные, правила входа и полномочия** остаются у владельца инстанса;
ядро даёт **формальные** механизмы — права, события, реестр модулей —
так, чтобы политика сообщества **лежала на свету**, а не в недокументированных
обходных путях.

**Для человечества это означает:** меньше цифровой зависимости от
непрозрачных посредников; больше **достоинства участника** (правила
читаемы), **устойчивости** (сообщество не исчезает с баном учётки
внешней сети) и **ответственности там, где она легитимна** — у Суверена
и граждан, а не у абстрактного «сервиса».

### Открытость и расширяемость

**Платформа KrwnOS и официальные модули проекта** — **открытое
программное обеспечение**: изучать, воспроизводить, улучшать и
разворачивать без чужого разрешения. Для **сторонних расширений**
(каталог, marketplace) мы закрепляем принцип **аудируемости**:
в каталоге по умолчанию — только то, чей **исходный код доступен на
чтение** под лицензией, совместимой с проверкой сообществом (в духе
[OSI](https://opensource.org/osd)). Закрытые «чёрные ящики» противоречат
миссии: суверенный аудит невозможен без читаемости.

**Автоматический разбор и модели машинного обучения** (включая большие
языковые модели) мы приветствуем как **ускорители внимательного аудита**
перед установкой: карта поверхности атаки, сеть, секреты, соответствие
манифесту прав. Они **не заменяют** человеческое решение и **не являются**
доказуемым «обезвреживанием». **Доверие к модулю выбирает Суверен;**
ядро **сдерживает ущерб** через минимальные привилегии, песочницу,
подписи и явный манифест (см. [`ROADMAP.md`](./ROADMAP.md), Horizon 3).

### Обязательства перед сообществом

Ниже — не маркетинговый список, а **критерии**, по которым мы просим
судить проект спустя годы:

1. **Фидуциарность.** Платформа не подменяет конституцию State и не
   вводит скрытых «суперправ» в обход заявленной модели разрешений.
2. **Открытость ядра и first-party модулей.** Код остаётся доступным
   для аудита и форка на условиях лицензии репозитория.
3. **Аудируемый каталог расширений.** Сторонние модули в официальном
   каталоге — с открытым исходником и лицензией, пригодной для проверки;
   ослабление этого принципа — только явное, публичное решение с
   обоснованием, а не тихий сдвиг.
4. **Суверенитет выбора.** Мы не навязываем конкретные модули как
   условие существования инстанса; зависимости и обновления —
   прозрачны насколько позволяет архитектура.
5. **Переносимость и наследие.** Снимки, экспорт и документированные
   пути восстановления развиваются так, чтобы **не похищать** историю
   сообщества при смене хостинга или поколения администраторов.
6. **Добровольная совместимость.** Связь между разными State — по
   согласию сторон (биржи, мосты, федерация), без обязательного
   центрального хозяина сети.
7. **Честность о пределах безопасности.** Автоматизация и ИИ —
   помощники; гарантии формулируются **точно**, без имитации
   формальной верификации там, где её нет.
8. **Доступность и язык.** По мере зрелости продукта — разумные усилия
   к доступному интерфейсу и к **многоязычию**, **не подменяя** местное
   право избирать официальный язык внутри State.

### Формулировки для наследия

1. *«KrwnOS **служит** Суверену: фидуциарий конституции, а не её
   тайный конкурент.»*
2. *«Доверие — через **читаемость**: видеть, проверять, форкать,
   уносить данные.»*
3. *«Машины **помогают** аудиту; **ответственность** — у людей;
   архитектура **ограничивает** беду.»*
...
Смысл, обязательства и долгий горизонт — в разделе **«Миссия»** выше.
