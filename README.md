...
- [x] Типовая модель ядра (`types/kernel.ts`)
- [x] Permissions Engine (наследование по Вертикали)
- [x] Module Registry + Event Bus
- [x] Prisma-схема (`State`, `VerticalNode`, `Membership`, `InstalledModule`)
...
- [x] Три tier-а установки: Sandbox / Pro / Cloud (concept + install scripts + compose)
- [x] Krwn CLI (login / module / vertical / invite / backup / status)
- [x] Magic-link invitations с SHA-256-hashing и QR-кодами
- [x] Passwordless credential registry (passkey / wallet / oauth / magic email)
- [x] Tunneling adapter (cloudflared / frp / ngrok / tailscale funnel)
- [x] Data isolation: schema-per-module
- [x] Backup engine (versioned snapshot JSON)
